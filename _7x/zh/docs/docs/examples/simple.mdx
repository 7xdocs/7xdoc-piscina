...
在这个示例中，我们创建了一个 Piscina 实例，该实例使用 worker 文件执行简单的加法操作。主脚本 (`index.js`) 创建 Piscina 实例并运行任务，而 worker 脚本 (`worker.js`) 定义要执行的任务。
...
你也可以在 [github](https://github.com/piscinajs/piscina/tree/current/examples/simple) 上查看此示例。
