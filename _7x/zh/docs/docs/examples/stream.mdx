...
此示例解释了如何将 Piscina 用于涉及流处理的更复杂场景。Piscina 允许使用 `MessagePort` 在主线程和 worker 线程之间进行高效的数据传输。

主脚本 (`index.mjs`) 设置了一个管道，该管道读取文件，通过 worker 线程处理其内容（将文本转换为大写），并输出结果。
...
流文件定义了扩展标准 Node.js 流的自定义流类，以便与 `MessagePort` 一起工作。这些类实现了主线程和 worker 线程之间的高效通信。
<Tabs>
<TabItem value="Javascript">
